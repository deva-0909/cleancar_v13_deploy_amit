            ...
            <CardContent className="space-y-4">
              ...
              {this.state.error && (
                <div className="p-4 bg-red-50 border border-red-200 rounded">
                  ...
                  {(this.state.error.message?.toLowerCase().includes('quota') || (this.state.error as any).name === 'QuotaExceededError') ? (
                    <>
                      <p className="text-sm text-red-800">The quota has been exceeded — browser storage is full.</p>
                      <button
                        className="mt-2 text-sm text-blue-700 underline font-medium block"
                        onClick={() => {
                          ['cc360_error_log','SYNC_RETRY_QUEUE','cc360_retry_queue','cc360_audit_trail'].forEach(k => {
