...
const generateEmployeeLedger = (empId: string): LedgerEntry[] => {
  ...
  const liveEmp = liveEmployees.find(e => (e.id || e.tempId) === empId);
  const emp = liveEmp
    ? { id: liveEmp.id || liveEmp.tempId, name: liveEmp.fullName, empCode: liveEmp.empCode || liveEmp.id, role: liveEmp.designation, department: liveEmp.department, joiningDate: liveEmp.joiningDate }
    : MASTER_EMPLOYEES.find((e) => e.id === empId);
  if (!emp) return [];
...
export function EmployeeLedger() {
  ...
  const liveEmpData = employeeDatabaseService.getAll();
  const liveSelected = liveEmpData.find(e => (e.id || e.tempId) === selectedEmployee);
  const employee = liveSelected
    ? { id: liveSelected.id || liveSelected.tempId, name: liveSelected.fullName, empCode: liveSelected.empCode || liveSelected.id, role: liveSelected.designation, department: liveSelected.department }
    : MASTER_EMPLOYEES.find((e) => e.id === selectedEmployee);
      ...
      <Card>
        ...
        <CardContent>
          <Select value={selectedEmployee} onValueChange={setSelectedEmployee}>
            ...
            <SelectContent>
              {(employeeDatabaseService.getAll().length > 0 ? employeeDatabaseService.getAll().map(e => ({ id: e.id || e.tempId, name: e.fullName, empCode: e.empCode || e.id })) : MASTER_EMPLOYEES).map((emp) => (
