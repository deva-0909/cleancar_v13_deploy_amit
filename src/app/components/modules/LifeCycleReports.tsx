...
export function LifeCycleReports() {
  // Use live employee data, fall back to LIVE_EMPLOYEES if service is empty
  const LIVE_EMPLOYEES = (() => {
    const live = employeeDatabaseService.getAll();
    return live.length > 0
      ? live.map(e => ({ ...e, id: e.id || e.tempId, name: e.fullName, empCode: e.empCode || e.id }))
      : LIVE_EMPLOYEES;
  })();
  ...
  const handleEmployeeClick = (empId: string) => {
    const employee = LIVE_EMPLOYEES.find((emp) => emp.empCode === empId);
    if (employee) {
