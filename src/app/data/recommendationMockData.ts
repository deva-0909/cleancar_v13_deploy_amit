// Mock Recommendation Data - Demonstrating all 10 diagnosis types

import { type Recommendation } from "./recommendationEngine";

export const MOCK_RECOMMENDATIONS: Recommendation[] = [
  // Diagnosis 1: Job Volume Shortfall - Washer Suresh Kumar (from Part 11 dummy data)
  {
    id: "rec-washer-001-job-shortfall-001",
    entityType: "Washer",
    entityId: "washer-001",
    ...
    whatIsHappening: "Suresh Kumar completed 477 jobs in March 2026 against a target of 546 — a shortfall of 69 jobs (12.6% below ideal). 1 zero-output day recorded (16 Mar — sick leave).",
    
    whyItMatters: "Salary CPW is ₹31.45 vs ideal ₹27.47 — ₹3.98 excess per wash. Total unrecovered salary excess: ₹3.98 × 477 = ₹1,898.46 this month. Annualised if pattern continues: ₹22,781.",
    
    actionSteps: [
      {
        step: 1,
        action: "Supervisor Ramesh to review Suresh's daily assignment schedule — confirm 21 jobs are assigned each working day.",
        owner: "Supervisor",
      },
      {
        step: 2,
        action: "Review sick leave pattern — this is Suresh's first CL in 2026. Monitor next month.",
        owner: "HR",
      },
      {
        step: 3,
        action: "If zone has insufficient subscription density for 21 daily jobs — expand CRM activity in PIN 395005.",
    ...
    expectedImpact: "Reaching 21 cars/day saves ₹3.98/wash × 21 × 26 = ₹2,174.28/month.",
    
    status: "Not Started",
    raisedOn: "2026-04-01T10:30:00Z",
    ...
    metrics: {
      ...
      financialImpact: 1898.46,
    },
  },
  
  // Recommendation 2 (Part 11): Quality Risk - Consumable Under-Consumption - Suresh Kumar
  {
  ...
  // Recommendation 3 (Part 11): Supervisor Underutilization - Ramesh Patel
  {
    ...
    whyItMatters: "The ₹8.01/wash supervisor excess × 2,336 team jobs = ₹18,711 in excess supervisor cost in March alone. Annualised: ₹2,24,532. Same impact exists for Supervisor Priya Mehta in PIN 395006 (3 washers).",
  ...
  // Recommendation 4 (Part 11): High Carry-Forward - Suresh Kumar Foam Shampoo
  {
    ...
    expectedImpact: "Pausing one month's issuance of 500ml × ₹0.88 = ₹440 in working capital freed. No expiry risk as Batch 001 expires well in future.",
...
/**
 * Get active recommendations (not completed)
 */
export function getActiveRecommendations(): Recommendation[] {
  return MOCK_RECOMMENDATIONS.filter(r => r.status !== "Completed");
}

/**
 * Get completed recommendations (resolved history)
 */
export function getCompletedRecommendations(): Recommendation[] {
  return MOCK_RECOMMENDATIONS.filter(r => r.status === "Completed");
}
...
/**
 * Get recommendations for specific period
 */
export function getRecommendationsForPeriod(period: string): Recommendation[] {
  return MOCK_RECOMMENDATIONS.filter(r => r.period === period);
}