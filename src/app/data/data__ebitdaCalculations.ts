...
export function getClothCost(planType: string): number {
  ...
  // Interior cloth (only for Water + Shampoo and Water + Shampoo + Wax)
  const interiorClothCost = (planType === "Water + Shampoo" || planType === "Water + Shampoo + Wax")
...
export function getEquipmentCost(planType: string): number {
  ...
  // Vacuum cleaner (only Water + Shampoo + Wax with interior service)
  const vacuumCostPerWash = planType === "Water + Shampoo + Wax"
