...
export const migratePayrollRuns = (): {
  ...
  try {
    ...
    // Step 1: Backup (skip if localStorage full — data still migrates in memory)
    const payrollBackupOk = backupPayrollData(existing);
    if (!payrollBackupOk) {
      logger.warn("Payroll backup skipped (localStorage full) — proceeding with migration");
    }
...
export const migrateSalaryStructures = (): { success: boolean; count: number; error?: string } => {
  try {
    ...
    // Step 1: Backup (skip if localStorage full — data still migrates in memory)
    const salaryBackupOk = backupSalaryData(existing);
    if (!salaryBackupOk) {
      logger.warn("Salary backup skipped (localStorage full) — proceeding with migration");
    }
