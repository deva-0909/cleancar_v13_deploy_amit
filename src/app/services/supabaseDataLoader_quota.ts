...
function safeWrite(key: string, data: any[]): void {
  ...
  for (const attempt of attempts) {
    try {
      // Check remaining quota before writing
      const estimatedSize = JSON.stringify(attempt).length * 2;
      let used = 0;
      for (const k of Object.keys(localStorage)) used += (localStorage.getItem(k) || '').length * 2;
      if (used + estimatedSize > 4 * 1024 * 1024) { // stop at 4MB to leave headroom
        console.warn(`[Supabase] Skipping ${key} — would exceed 4MB storage limit`);
        return;
      }
