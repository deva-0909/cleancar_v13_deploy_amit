Update the transaction type data model to support sub-types and custom categories.
 
STEP 1 — Update GSTTransaction interface in gstComplianceService.ts.
Open src/app/services/gstComplianceService.ts.
 
Find the GSTTransaction interface and add these two new optional fields
after the existing transactionType field:
 
  transactionType: "Sale" | "Purchase" | "Credit Note" | "Debit Note";
  transactionSubType?: string;     // ← ADD: e.g. "Capital Goods", "Rent", "Consumable"
  transactionCategory?: string;    // ← ADD: e.g. "PURCHASE_CAPITAL", "EXPENSE_RENT"
 
Also update the transactionType union to add "Expense":
  transactionType: "Sale" | "Purchase" | "Expense" | "Credit Note" | "Debit Note";
 
STEP 2 — Create a new file: src/app/config/gstTransactionTypes.ts
 
This file is the SINGLE source of truth for all transaction types and
their sub-types. It also stores custom user-defined sub-types.
...
// Functions to get sub-types for a given parent type
export function getSubTypesForParent(parentType: string): TransactionSubType[] {
  ...
  try {
    ...
    const customTypes = DataService.get("CUSTOM_TRANSACTION_SUB_TYPES") as TransactionSubType[];
...
export function getSubTypeById(id: string): TransactionSubType | undefined {
  return SYSTEM_TRANSACTION_SUB_TYPES.find(st => st.id === id);
}
